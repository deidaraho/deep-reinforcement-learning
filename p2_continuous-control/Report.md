...
- **./Continuous_Control.ipynb**: the python notebook to train the agent with DDPG
- **./model.py**: the PyTorch model for actor and critic
- **./ddpg.py**: the python implementation of ddpg agents
- **./checkpoint_actor.pth**: actor network's final weight
- **./checkpoint_critic.pth**: critic network's final weight
- **./scores.npy**: average score recording during training
...
In this case, the system needs to train both a actor for decision making and a critic for reward prediction.
...
In this project, I used the soft update rate 99% instead.
...
All of the above mentioned techniques were incorporated. The entire implementation was done in PyTorch. Also I found 1, the leaky-Relu is better than Relu, 2, batch normalization is useful for actor to handle states.
